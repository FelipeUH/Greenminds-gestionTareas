...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  // Obtener conteo de proyectos del usuario
  ...
  // Calcular tiempo promedio de tarea (simplificado - usando horas estimadas por ahora)
  const completedTasks = allTasks.filter(
    (t) => t.status === "done" && t.actual_hours
  );
  ...
  const averageHours =
    completedTasks.length > 0 ? totalHours / completedTasks.length : 0;
  const averageDays = Math.floor(averageHours / 8); // Assuming 8 hours per day
  const remainingHours = averageHours % 8;

  // Obtener tareas vencidas (simplificado - usando fecha actual)
  ...
  const overdueStats = {
    unassigned:
      overdueTasks?.filter((t) => t.status === "unassigned").length || 0,
